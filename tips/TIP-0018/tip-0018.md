...
4. [New concepts of output design](#new-concepts)
    ...
    - [Limited Addresses](#limited-addresses)
...
[TIP-20](../TIP-0020/tip-0020.md) replaces aforementioned
...
It is possible to limit the use of an _Ed25519 Address_. For these limited addresses, the first three bits of the _Address Type_ define its capabilities using bitflags:
- If the bit with the value `0x01` is set, the address can only be used for "simple transactions" without additional conditions.
- If the bit with the value `0x02` is set, only IOTA coins can be sent to this address.
- The bit with value `0x04` is currently reserved and must always be zero.

The `Address Type` of `0` remains unchanged and represents a regular _Ed25519 Address_ without any limitations.

It is important to note that these limitations are only enforced by the _Address Type_. Therefore, it is theoretically possible to use the same Ed25519 private key for both limited and regular addresses.
...
<details open>
    ...
    <table>
        ...
        <tr>
            ...
            <td>uint8</td>
            <td>
                Set to <strong>value 0, 1, 2 or 3</strong> to denote an <i>Ed25519 Address</i>.
            </td>
        </tr>
        <tr>
            <td>PubKeyHash</td>
...
- If the `Address Type` has the bit with value `0x01` set:
  - `Output Type` must be `3` (_Basic Output_) and
  - `Unlock Conditions Count` must be `1` (exactly one _Address Unlock Condition_).
- If the `Address Type` has the bit with value `0x02` set:
...
An <i>NFT Unlock</i> looks and behaves like an <i>Alias Unlock</i>, but the referenced input at the index must
